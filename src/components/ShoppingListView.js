...
export class ShoppingListView {
  ...
  getRecipesFromMealPlan(allRecipes, mealPlan) {
    ...
    console.log(`  📋 Extracting recipes from meal plan with ${mealPlan.days.length} days`);
        ...
        if (meal && meal.name) {
          ...
          if (!addedNames.has(recipeName)) {
            ...
            if (recipe) {
              ...
              console.warn(`  ⚠️ Recipe not found and no ingredients: "${meal.name}" (day ${dayIdx + 1}, ${mealType})`);
              missingCount++;
            }
          }
        }
    ...
    console.log(`  ✅ Extracted ${usedRecipes.length} unique recipes (${catalogCount} from storage, ${newRecipeCount} with inline ingredients, ${missingCount} missing)`);
